...
const load = () => {
    const insertEmoji = (emoji: string, name: string) => {
    ...
    for (const [emojiDescriptor, ...names] of rawEmojiData as EmojiDataEntry[]) {
        ...
        for (const nameDescriptor of names) {
            const [nameTemplate, flags] = typeof nameDescriptor === 'string' ? [nameDescriptor, 0] : nameDescriptor;
...
export const translateNamedEmoji = (content: string) => {
    const match = /^:([^\s:]+?(?:::skin-tone-\d)?):/.exec(content);
    if (!match)
        return { content, offset: 0, name: undefined };
...
export const findEmoji = (content: string) => {
    let buffer = '';
    let part = '';
    ...
    for (const character of content) {
        ...
        if (!part || modifier.test(character) || (suffix.test(character) && (emojiToNameMap.has(part + character) || emojiToNameMap.has(buffer + character)))) {
            buffer += character;
            part += character;
            continue;
        }
...
export const trimToNearestNonSymbolEmoji = (value: string) => value.replace(/(?:\u2139\ufe0f|\d\ufe0f?\u20e3).*/, '');

export const getEmojiName = (emoji: string) => emojiToNameMap.get(emoji);

export const getEmojiWithName = (emoji: string) => nameToEmojiMap.get(emoji);

const emojiBaseUrl = 'https://cdn.jsdelivr.net/gh/jdecked/twemoji@14.1.2/assets/svg';

export const getEmojiImageUrl = (emoji: string) => {
    let codePoints = [];
